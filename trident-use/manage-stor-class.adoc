...
以下是您可以使用JSON为Astra Trident后端对象输出的问题示例 `tridentctl`。这将使用 `jq`实用程序、您可能需要先安装该实用程序。
...
NOTE: Astra Trident会对其创建的卷强制执行空白 `fsType`操作。对于iSCSI后端、建议在StorageClass中强制实施 `parameters.fsType`。您应删除现有StorageClasses、然后使用指定的重新创建它们 `parameters.fsType`。
