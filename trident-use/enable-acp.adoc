...
如果将之前未安装 Astra Trident 的集群添加到 Astra Control Service，此集群将被标记为 `Eligible`。将此集群添加到 Astra Control 后，将自动启用 Astra Control Provisioner。
...
. （仅限自定义注册表）按照以下步骤将映像移动到自定义注册表。如果您不使用注册表，请按照 <<first-step,下一节>> 中的 Trident 操作员步骤操作。
+
...
. [[first-step]]使用您最初使用的安装方法在 Astra Trident 中启用 Astra Control Provisioner：
+
[role="tabbed-block"]
====
.Asta三端操作员
--
.. https://docs.netapp.com/us-en/trident/trident-install/kubernetes-deploy-operator.html#step-1-download-the-trident-installer-package["下载Asta三端安装程序并解压缩"^]。
...
.Tridentctl
--
.. https://docs.netapp.com/us-en/trident/trident-install/kubernetes-deploy-tridentctl.html#step-1-download-the-trident-installer-package["下载Asta三端安装程序并解压缩"^]。
