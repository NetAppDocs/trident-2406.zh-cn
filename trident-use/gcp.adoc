...
Astra Trident可以通过link:https://cloud.google.com/architecture/partners/netapp-cloud-volumes/service-types["服务类型"^]以下两种方式之一创建Cloud Volumes Service卷：

* *CVS-Performance*：默认的Astra Trident服务类型。这种性能优化的服务类型最适合重视性能的生产工作负载。CVS-Performance服务类型是一种硬件选项、支持的卷大小至少为100 GiB。您可以选择link:https://cloud.google.com/architecture/partners/netapp-cloud-volumes/service-levels#service_levels_for_the_cvs-performance_service_type["三个服务级别"^]以下选项之一：
+
** `standard`
** `premium`
** `extreme`
