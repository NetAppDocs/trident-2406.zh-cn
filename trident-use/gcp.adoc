...
Asta Trident提供了 `gcp-cvs` 用于与集群通信的驱动程序。支持的访问模式包括：_ReadWriteOnce_(RWO)、_ReadOnlyMany_(ROX)、_ReadWriteMany_(rwx)、_ReadWriteOncePod_(RWOP)。
...
* *CVS-Performance*：默认的 Astra Trident 服务类型。这种性能优化的服务类型最适合重视性能的生产工作负载。CVS-Performance 服务类型是支持最小 100 GiB 大小的卷的硬件选项。您可以选择以下选项之一 link:https://docs.cloud.google.com/netapp/volumes/docs/discover/overview["三个服务级别"^]：
+
** `standard`
** `premium`
** `extreme`
...
* *CVS*：CVS 服务类型提供高分区可用性，性能水平有限至中等。CVS 服务类型是一种软件选项，它使用存储池来支持小至 1 GiB 的卷。存储池最多可包含 50 个卷，其中所有卷共享池的容量和性能。您可以选择以下选项之一 link:https://docs.cloud.google.com/netapp/volumes/docs/discover/overview["两个服务级别"^]：
+
** `standardsw`
...
.您需要的内容
要配置和使用  https://www.netapp.com/google-cloud/netapp-volumes/["适用于 Google Cloud 的 Cloud Volumes Service"^] 后端，您需要以下内容：
...
| `storageClass` | 用于指定CVS服务类型的可选参数。 `software`用于选择CVS服务类型。否则，Astra Trident会采用CVS性能服务类型(`hardware`)。 |  
...
| `apiRegion` | Astra Trident创建Cloud Volumes Service 卷的Google云区域。创建跨区域Kubnetes集群时、在中创建的卷 `apiRegion`可用于在多个Google Cloud区域的节点上计划的工作负载。跨区域流量会产生额外成本。 |  
