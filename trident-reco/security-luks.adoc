...
在Astra Trident中，根据的建议，经过LUN加密的卷使用AES-XTS-Pastr64 cypher和模式link:https://csrc.nist.gov/publications/detail/sp/800-38e/final["NIST"^]。
...
要导入LUN卷、必须在后端将设置 `luksEncryption`为(`true`。该 `luksEncryption`选项会告诉Astra Trident卷是符合LUN (`true`(兼容)还是不符合LUN (`false`(兼容)，如以下示例所示。
