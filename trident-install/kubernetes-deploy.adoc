...
无论是手动部署还是使用 Helm，Trident 操作员都是简化安装和动态管理 Astra Trident 资源的好方法。您甚至可以link:kubernetes-customize-deploy.html["自定义Trident操作员部署"]使用 `TridentOrchestrator`自定义资源 (CR) 中的属性。
