...
要进行试驾，请使用现成的实验室映像请求访问"轻松部署和克隆容器化工作负载的永久存储"link:https://www.netapp.com/test-drives/["NetApp 试用"^]。测试驱动提供了一个安装并配置了三节点 Kubernetes 集群和 Astra Trident 的沙盒环境。这是熟悉 Astra Trident 并探索其功能的好方法。
