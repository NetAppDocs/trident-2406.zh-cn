...
summary: '要创建卷、请使用标准 `docker volume`命令以及根据需要指定的Asta Trident驱动程序名称。' 
---
= 使用卷
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
您可以使用标准命令以及根据需要指定的Asta Trident驱动程序名称轻松创建、克隆和删除卷 `docker volume`。
...
使用 `ontap-nas`、、 `ontap-san` `solidfire-san`和 `gcp-cvs storage drivers`时，Astra Trident可以克隆卷。使用或 `ontap-nas-economy`驱动程序时 `ontap-nas-flexgroup`、不支持克隆。从现有卷创建新卷将创建新快照。
...
如果容器没有分区、并且Asta Trident支持其文件系统、则您可以使用Trident *仅通过容器访问外部创建的块设备(或其克隆)(例如： `ext4`无法通过Asta Trident访问格式化的 `/dev/sdc1`)。
